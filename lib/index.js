...
module.exports = async () => {
  ...
  console.log(`${chalk.green('i')} ${chalk.bold(`当前项目版本(${chalk.blueBright(pkg.version)})`)}`)
  ...
  if (!lastTagConfirmed) {
    const choices = tagNames.slice(0, 15).map(tagName => {
      return {
        name: tagName,
        value: tagName
      }
    ...
    if (choices.length !== 0) {
      // 如果有历史 tag，继续选择
      ({ lastTagName } = await inquirer.prompt([
        {
          name: 'lastTagName',
          message: '选择上一个Tag名称: ',
          type: 'list',
          choices: tagNames.slice(0, 15).map(tagName => {
            return {
              name: tagName,
              value: tagName
            }
  ...
  // 判断新的 Tag 名称是否存在
  let { stdout: newTagNameStdout } = await execa.command(`git tag --list ${newTagName}`)
  if (newTagNameStdout.trim() !== '') {
    console.log(chalk.redBright(`\n✘ ︎${newTagName} 已存在，请重新选择\n`))
    process.exit()
  }
